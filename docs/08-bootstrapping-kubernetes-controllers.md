...
The commands in this lab must be run on each controller instance: `master1`, and `master2`. Login to each controller instance using SSH Terminal.
...
ExecStart=/usr/local/bin/kube-apiserver \\
  ...
  --etcd-servers=https://192.168.1.14:2379,https://192.168.1.15:2379 \\
  --event-ttl=1h \\
...
somesh@k8s-ha-master2:~$ kubectl get componentstatuses --kubeconfig admin.kubeconfig
NAME                 STATUS    MESSAGE             ERROR
controller-manager   Healthy   ok
scheduler            Healthy   ok
etcd-0               Healthy   {"health":"true"}
