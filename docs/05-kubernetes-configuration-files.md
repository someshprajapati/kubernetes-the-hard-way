...
Each kubeconfig requires a Kubernetes API Server to connect to. To support high availability the IP address assigned to the load balancer will be used. In our case it is `192.168.1.18`

```
somesh@k8s-ha-master1:~$ LOADBALANCER_ADDRESS=192.168.1.18
```
...
```
somesh@k8s-ha-master1:~$ for instance in worker1 worker2; do
